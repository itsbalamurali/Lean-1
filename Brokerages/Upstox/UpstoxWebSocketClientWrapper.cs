...
{
    //<summary>
    // UpstoxWebSocketClientWrapper ..
    //</summary>
    public class UpstoxWebSocketClientWrapper
    {
            ...
            lock (_locker)
            {
                if (_cts == null)
                {
                    ...
                    _taskConnect = Task.Factory.StartNew(
                        () =>
                        {
                            Log.Trace("UpstoxWebSocketClientWrapper connection task started.");
                            ...
                            {
                                Log.Error(e, "Error in UpstoxWebSocketClientWrapper connection task");
                            }

                            Log.Trace("UpstoxWebSocketClientWrapper connection task ended.");
                        },
                        _cts.Token);
                }
            }
        ...
        public void Close()
        {
            ...
            {
                Log.Error($"UpstoxWebSocketClientWrapper.Close(): {e}");
            }
        ...
        {
            Log.Error(e.Exception, $"UpstoxWebSocketClientWrapper.OnError(): (IsOpen:{IsOpen}, State:{_client.State}): {e.Message}");
            Error?.Invoke(this, e);
        }
        ...
        {
            Log.Trace($"UpstoxWebSocketClientWrapper.OnOpen(): Connection opened (IsOpen:{IsOpen}, State:{_client.State}): wss://ws.kite.trade ...");
        ...
        {
            Log.Trace($"UpstoxWebSocketClientWrapper.OnClose(): Connection closed (IsOpen:{IsOpen}, State:{_client.State}):  wss://ws.kite.trade ...");
        ...
        private async Task HandleConnection(CancellationTokenSource connectionCts)
        {
            ...
            {
                Log.Trace("UpstoxWebSocketClientWrapper.HandleConnection(): Connecting to  wss://ws.kite.trade ....");

                try
                {                    
                    _client.Options.SetRequestHeader("X-UpstoxAPI-Version", "3");
                        ...
                        !connectionCts.IsCancellationRequested)
                    {
                        ...
                        {
                            Log.Trace("UpstoxWebSocketClientWrapper.HandleConnection(): WebSocketMessageType.Close");
                            return;
                        }
