...
{
    /// <summary>
    /// Kraken REST API implementation
    /// </summary>
    public class KrakenRestApiClient : IDisposable
    {
        ...
        /// </summary>
        public event EventHandler<KrakenOrderSubmitEventArgs> OrderSubmit;
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="KrakenRestApiClient"/> class.
        /// </summary>
        /// <param name="symbolMapper">The symbol mapper.</param>
        /// <param name="securityProvider">The holdings provider.</param>
        /// <param name="apiKey">The Kraken API key</param>
        /// <param name="apiSecret">The The Kraken API secret</param>
        public KrakenRestApiClient(SymbolPropertiesDatabaseSymbolMapper symbolMapper, ISecurityProvider securityProvider, string apiKey, string apiSecret,string apiConnectName)
        {
        ...
        public bool PlaceOrder(Order order)
        {
            ...
            switch (order.Type)
            {
                case OrderType.Limit:
                    body["type"] = (order.Properties as KrakenOrderProperties)?.PostOnly == true
                        ? "LIMIT_MAKER"
                        : "LIMIT";
                ...
                case OrderType.StopLimit:
                    ...
                    break;
                default:
                    throw new NotSupportedException($"KrakenBrokerage.ConvertOrderType: Unsupported order type: {order.Type}");
            }
        ...
        public bool SessionKeepAlive()
        {
            ...
            {
                throw new Exception("KrakenBrokerage:UserStream. listenKey wasn't allocated or has been refused.");
            }
        ...
        public void CreateListenKey()
        {
            ...
            if (response.StatusCode != HttpStatusCode.OK)
            {
                throw new Exception($"KrakenBrokerage.StartSession: request failed: [{(int)response.StatusCode}] {response.StatusDescription}, Content: {response.Content}, ErrorMessage: {response.ErrorMessage}");
            }
        ...
        private decimal GetTickerPrice(Order order)
        {
            ...
            if (tickerPrice == 0)
            {
                ...
                if (ticker == null)
                {
                    throw new KeyNotFoundException($"KrakenBrokerage: Unable to resolve currency conversion pair: {order.Symbol}");
                }
        ...
        private static string ConvertOrderDirection(OrderDirection orderDirection)
        {
            ...
            throw new NotSupportedException($"KrakenBrokerage.ConvertOrderDirection: Unsupported order direction: {orderDirection}");
        }
        ...
        private string ConvertResolution(Resolution resolution)
        {
            ...
            {
                throw new ArgumentException($"KrakenBrokerage.ConvertResolution: Unsupported resolution type: {resolution}");
            }
        }
        ...
        private void OnOrderSubmit(Messages.NewOrder newOrder, Order order)
        {
            try
            {
                OrderSubmit?.Invoke(
                    this,
                    new KrakenOrderSubmitEventArgs(newOrder.Id, order));
