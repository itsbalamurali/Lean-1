...
namespace QuantConnect.Brokerages.Kraken
{
    ...
    public partial class KrakenBrokerage : BaseWebsocketsBrokerage, IDataQueueHandler
    {
        ...
        private readonly SymbolPropertiesDatabaseSymbolMapper _symbolMapper = new SymbolPropertiesDatabaseSymbolMapper(Market.Kraken);
        ...
        private readonly KrakenRestApiClient _apiClient;
        ...
        public KrakenBrokerage(string connectKey, string connectSecret,string keyName, IAlgorithm algorithm, IDataAggregator aggregator)
            : base(WebSocketBaseUrl, new WebSocketClientWrapper(), null, connectKey, connectSecret, "Kraken")
        {
            ...
            _apiClient = new KrakenRestApiClient(
                _symbolMapper,
                algorithm?.Portfolio,
                connectKey,
                connectSecret,
                ...
                switch (item.Type.LazyToUpper())
                {
                    ...
                    default:
                        OnMessage(new BrokerageMessageEvent(BrokerageMessageType.Error, -1,
                            "KrakenBrokerage.GetOpenOrders: Unsupported order type returned from brokerage: " + item.Type));
                        continue;
                }

                order.Quantity = item.Quantity;
                order.BrokerId = new List<string> { item.Id };
                order.Symbol = _symbolMapper.GetLeanSymbol(item.Symbol, SecurityType.Crypto, Market.Kraken);
        ...
        private void OnOrderSubmit(KrakenOrderSubmitEventArgs e)
        {
